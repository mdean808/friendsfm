<script lang="ts">
  ...
  onMount(() => {
    ...
    player = new Audio($audialAnswer?.preview_url);
  ...
  audialAnswer.subscribe((value) => {
    if (!value) return;
    player?.remove();
    player = new Audio(value.preview_url);
  });
  audialSongPaused.subscribe((value) => (paused = value));

  const playSong = () => {
    audialSongPaused.set(false);
    player.play();
    timeElapsed = '0:00';
